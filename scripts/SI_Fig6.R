...
#ggsave(plot = p, filename = "figures/food-additives-species-bubbles_FA1_RA5.svg", width = 6.5, height = 3.5, bg = "transparent")
...
extract_substrate <- function(target) {
  ...
  filtered_metadata <- metadata %>%
    ...
    mutate(
      ...
      prefix = paste(participant, substrate, timepoint, sep = "_"), 
      newID = paste( participant, timepoint, sep = "_")
    ) %>%
    arrange(participant, substrate, timepoint) %>%  
    mutate(prefix = factor(prefix, levels = unique(paste(participant, substrate, timepoint, sep = "_"))))
  ...
  abundance_long <- filtered_abundance %>%
    ...
    mutate(
      abundance = abundance,
      timepoint = factor(timepoint, levels = c("T1", "T2", "T3"))) %>% 
    filter(cazymes %in% names(target_colors))
  
  # Make wide
  abundance_wide <- abundance_long %>%
    select(cazymes, newID, cazymes, substrate, abundance) %>%
    ...
    replace_na(list(F = 0, GM = 0, GA = 0)) %>%
    mutate(change = (get(target) - F),
           prefix = paste0(
             newID, "_", target ))
  
  sum_data <- abundance_wide %>%
    group_by(prefix, newID, cazymes) %>%
    filter(change > 0) %>%
    summarise(sum_change = sum(change, na.rm = TRUE), .groups = "drop") %>%
    mutate(cazymes = factor(cazymes, levels = names(target_colors)))
  
  sum_data
}

merged_data <- bind_rows(extract_substrate("GM"), extract_substrate("GA"))

target_colors <- c(GM_colors, GA_colors)

p <- ggplot(merged_data, aes(x = prefix, y = log2(sum_change +1), fill = cazymes)) +
...
#ggsave("figures/food-additives.svg", p, width = 10, height = 3, bg = "transparent")
